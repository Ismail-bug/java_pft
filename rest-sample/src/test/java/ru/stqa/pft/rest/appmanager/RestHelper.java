...
public class RestHelper {
    ...
    public Set<Issue> getIssues() throws IOException {
        String json = getExecutor().execute(Get(app.getProperty("bugify.URL") + "issues.json")).returnContent().asString();
    ...
    public int createIssue(Issue issue) throws IOException {
        String json = getExecutor().execute(Post(app.getProperty("bugify.URL") + "issues.json").bodyForm(new BasicNameValuePair("subject", issue.getSubject()), new BasicNameValuePair("description", issue.getDescription()))).returnContent().asString();
        JsonElement parsedJson = new JsonParser().parse(json);
